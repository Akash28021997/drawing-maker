...
const PreviewComponent = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Load and embed the Times New Roman font
      const fontUrl = "times-new-roman.ttf"; // Path to Times New Roman font file
      ...
      for (let index = 0; index < photos.length; index++) {
        ...
        page.drawText(`NAME: ${name}`, {
          ...
          size: 12,
          color: rgb(0, 0, 0),
          font: timesNewRomanFont, // Use Times New Roman font
        });
  ...
  const handleDownloadDOC = async () => {
    try {
      if (!photos || photos.length === 0) {
        throw new Error("No images found to generate DOCX.");
      }

      // Preload images before passing to ImageRun
      const imagesData = await Promise.all(
        photos.map(async (photo) => {
          return await photo.arrayBuffer();
        })
      );
      ...
      const doc = new Document({
        ...
        sections: photos.map((photo, index) => ({
          properties: {},
          children: [
            // Header with Free Formatting Alignment
            new Paragraph({
              children: [
                new TextRun({
                  text: `NAME: ${name}`,
            ...
            // Spacer (Adding 10 lines of space to push image down)
            ...Array(7).fill(new Paragraph("")),
            ...
            // Spacer (Adding 10 lines of space before footer)
            ...Array(7).fill(new Paragraph("")),

            // Footer (Attorney Info) Right Bottom Corner with Proper Spacing
            new Paragraph({
              alignment: "right",
              children: [
                new TextRun({
                  text: "MOHAN RAJKUMAR DEWAN, IN/PA-25",
                  bold: true,
                  size: 24,
                }),
              ],
            ...
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: "APPLICANT'S PATENT ATTORNEY",
                  bold: true,
                  size: 24,
                }),
              ],
      ...
      saveAs(docBuffer, `${number}-${name}.docx`);

      toast({
        title: "DOCX Downloaded.",
        description: `The Word document ${number}-${name}.docx has been successfully downloaded.`,
        status: "success",
        duration: 9000,
        isClosable: true,
      });
    } catch (error) {
      console.error("Error handling DOCX generation:", error);
      toast({
        title: "Download Failed.",
        description: "Error generating DOCX file. Try again.",
        status: "error",
        duration: 9000,
        isClosable: true,
      });
        ...
        {photos.map((photo, index) => (
          ...
          >
            ...
            <Image
              ...
              alt={`Uploaded ${index}`}
              maxW="100%"
              maxH="800px" // Adjust to fit content within the A4 height
              mx="auto"
              mb={4}
            />
            ...
            <Flex direction="column" align="flex-end" mt={4} mb={2}>
              ...
              />
              {/* Footer Text */}
              <Box textAlign="right" fontFamily="'Times New Roman', serif">
